...
class Logger {
    ...
    logTo(log, pre) {
        ...
        let fileLine = '';
        if (Config.logOpenFileLine) {
            try {
                throw new Error('logErr');
            } catch(e) {
                let arr = e.stack.split('(');
                arr = arr[3].split(')');
                if (Config.logOpenFilePath) {
                    fileLine = `[${arr[0]}]`;
                } else {
                    arr = arr[0].split(path.sep);
                    fileLine = `[${arr[arr.length - 1]}]`;
                }
            }
        }
        let str = `${year}-${month}-${date} ${toTwiceNumStr(time.getHours())}:${toTwiceNumStr(time.getMinutes())}:${toTwiceNumStr(time.getSeconds())}.${toThreeNumStr(time.getMilliseconds())} ${fileLine}[${pre}] ${log}`;
